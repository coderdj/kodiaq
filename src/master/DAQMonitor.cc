...
void DAQMonitor::PollNetwork()
{
  ...
  while(m_DAQStatus.NetworkUp){    
    ...
    for(unsigned int x=0;x<m_DAQStatus.Slaves.size();x++){
      if(difftime(fCurrentTime,m_DAQStatus.Slaves[x].lastUpdate)>10.){
	stringstream errtxt;
	errtxt<<"Dispatcher hasn't received a response from node "<<m_DAQStatus.Slaves[x].name<<" for at least 10 seconds.";
...
int DAQMonitor::Start(string user, string comment, koOptions *options)
{
  ...
  stringstream mess;
  mess<<"DAQ started by "<<user<<" : "<<comment;  
  if(m_Mongodb!=NULL){
...
int DAQMonitor::Stop(string user,string comment)
{
  ...
  stringstream mess;
  mess<<"DAQ stopped by "<<user<<" : "<<comment;
  if(m_Mongodb!=NULL){
    m_Mongodb->SendLogMessage(mess.str(),KOMESS_STATE);
